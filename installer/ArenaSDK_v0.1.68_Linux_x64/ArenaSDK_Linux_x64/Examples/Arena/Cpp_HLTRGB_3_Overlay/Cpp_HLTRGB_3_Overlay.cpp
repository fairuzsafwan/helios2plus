...
// file name
#define FILE_NAME_OUT "/temp/calibration_helios_triton/overlay.ply" //"Images\\Cpp_HLTRGB_3_Overlay.ply"

#define FILE_NAME_DEPTHMAP_16UC1 "/temp/calibrated_depthmap/depthmap_16UC1_calibrated.png"
#define FILE_NAME_DEPTHMAP_32FC1 "/temp/calibrated_depthmap/depthmap_32FC1_calibrated.tiff"
#define FILE_NAME_DEPTHMAP_8UC1 "/temp/calibrated_depthmap/depthmap_8UC1_calibrated.png"
...
void OverlayColorOnto3DAndSave(Arena::IDevice* pDeviceTRI, Arena::IDevice* pDeviceHLT)
{
	...
	for (int i = 0; i < width * height; i++)
	{
		...
		if (Z < 65535)
		{
			//Back projection to 2D image plane
			img_array[i] = Z;
		}
	...
	try
	{
		...
		cv::Mat cv_image2 = cv::Mat(480, 640, CV_32FC1, img_array);
		std::cout << "img_array: " << img_array[0] << std::endl;

		//cv::imwrite(FILE_NAME_DEPTHMAP_32FC1, cv_image2);
		//cv::Mat cv_image_16UC1 = cv_image2.clone();
		//cv::Mat cv_image_8UC1 = cv_image.clone();
		//Convert from 32FC1 to 16UC1
	
		cv_image2.convertTo(cv_image2, CV_16UC1);
		cv::imwrite(FILE_NAME_DEPTHMAP_16UC1, cv_image2);
	}
	catch(...) 
	{
	...
	// cv_image_8UC1.convertTo(cv_image_8UC1, CV_8UC1);
	// cv::imwrite(FILE_NAME_DEPTHMAP_8UC1, cv_image_8UC1);

	std::cout << "Camera Matrix: " << cameraMatrix << std::endl;
	std::cout << "Camera Matrix2: " << cameraMatrix.at<double>(0,0)<< std::endl;
	std::cout << "focal_X: " << focal_X << std::endl;

	std::cout << "Length of img_array: " << sizeof(img_array) << std::endl;
